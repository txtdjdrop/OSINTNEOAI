"""
cross_reference_master_accounts_v2.py — Handles ARRAY<STRING> fields cleanly in BigQuery SQL queries.
"""
import os
import json
...
def run_crossref():
    ...
    for dataset_id, table_id in TABLES_TO_SEARCH:
        ...
        try:
            ...
            # Exclude ARRAY or RECORD fields
            string_cols = [f.name for f in table.schema if f.field_type in ("STRING", "TEXT") and f.mode != "REPEATED"]
            if not string_cols:
                continue
