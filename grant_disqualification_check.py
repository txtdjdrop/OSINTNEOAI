...
# Load all candidate entities from multiple sources
def load_csv(path):
...
# Build a master map of entities -> disqualification reasons
...
def normalize(name):
    return str(name).upper().strip().replace(",", "").replace(" LLC", " LLC").replace("  ", " ")

# Rule 1: Suspicious HB network — LLCs with property in HB area + PPP loans out of state
for row in suspicious_network:
    ...
    # Apply rules
    mail_state = str(row.get("property_mail_state", "")).upper().strip()
    if mail_state and mail_state != "CA":
        entities[name]["reasons"].add("out_of_state_mail")

    if "LLC" in name or "INC" in name or "CORP" in name:
...
# Rule 2: PPP RICO matches — multiple loans / forgiven amounts
for row in ppp_rico:
...
# Rule 3: Church entities flagged from PPP 150k+
for row in church_entities:
    ...
    entities[name]["properties"]["source"].append(row.get("source", ""))

    state = str(row.get("state", "")).upper().strip()
    if state and state != "CA":
        entities[name]["reasons"].add("out_of_state_entity")

    # Churches that also took PPP may be flagged if not actually non-profit / local
    entities[name]["reasons"].add("ppp_church_entity")

# Rule 4: RICO evidence matrix
...
# Write output CSV
output_path = os.path.join(WORK_DIR, "grant_disqualified_entities.csv")
...
print(f"Total entities reviewed: {len(entities)}")
print(f"Disqualified entities written to: {output_path}")
