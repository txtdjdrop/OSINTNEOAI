...
SECRET_PATTERNS = {
    "Google / Gemini API Key": re.compile(r"\bAI" + r"za[0-9A-Za-z\-_]{35}\b"),
...
def scan_text_content(content: str, rel_path: str) -> List[Tuple[int, str, str]]:
    ...
    for idx, line in enumerate(lines, start=1):
        # Ignore comments or test fixtures with placeholder markers
        if any(marker in line for marker in ["REDACTED", "placeholder", "fake", "mock", "example", "dummy"]):
            continue
    ...
    for line in res.stdout.splitlines():
        if line.startswith("+++ b/"):
            ...
            if current_file in WHITELISTED_PATHS:
                line_number += 1
                continue
            if any(marker in added_line for marker in ["REDACTED", "placeholder", "mock", "dummy", "example"]):
                line_number += 1
                continue
            for name, pattern in SECRET_PATTERNS.items():
